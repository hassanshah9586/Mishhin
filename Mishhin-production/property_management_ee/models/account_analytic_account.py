...
class AccountAnalyticAccount(models.Model):
    ...
    @api.depends('invc_id', 'invc_id.payment_state')
    def compute_amount_return(self):
        ...
        for data in self:
            if data.invc_id.payment_state in ['in_payment', 'paid']:
    ...
    def button_receive(self):
        ...
        context.update({'close_after_process': True, 'account_deposit_received': True})
        for tenancy_rec in self:
            ...
            payment_id = self.env['account.payment'].with_context(context).create(vals)
        return {
            'view_mode': 'form',
            'view_id': acc_pay_form.id,
            'view_type': 'form',
