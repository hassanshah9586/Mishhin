...
class CrmLead(models.Model):
    ...
    client_type = fields.Selection(
        [('residential', 'Residential'), ('corporate', 'Corporate')], default='residential', string='Client Type')
    ...
    def makecontract(self):
        ...
        for make in self:
            ...
            for case in lead_obj.browse(data):
                if not case.property_id and case.is_rent:
                ...
                if case.is_rent:
                    ...
                    tenant_id.write({'civil_id': case.civil_id})
                    if case.tenancy_id:
                        raise ValidationError(_('For this lead already contract created.'))
                vals = {
                    'name': case.name,
                ...
                case.lead_attachment_ids.write({'tenant_id': tenant_id.id})
                case.write({
                    'description': 'account.analytic.account,%s' % new_id, 'tenancy_id': new_id.id})
