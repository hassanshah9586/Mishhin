...
class PropertyWebsite(http.Controller):
    ...
    def property_detail(self, property, **post):
        ...
        account_asset_asset_obj = request.env['account.asset'].sudo().search([])
    ...
    @http.route(['/contactus/get_create_lead'], type='json', auth="public", website=True)
    def get_create_lead(self, **kw):
        token_number = kw.get('token_number', False)
        if token_number:
            lead = request.env['crm.lead'].sudo().search([('lead_token', '=', token_number)])
            if lead and lead.client_type:
                return {
                    'id': lead.id,
                    'rent_type': lead.client_type,
                    'incomplete_document_list': [attach.document_type for attach in lead.lead_attachment_ids.filtered(lambda i: i.state in ('rejected','draft'))],
                }
        return False
    ...
    def crmcontactus(self, **kwargs):
        ...
        residential_dict = kwargs.get('residential_dict')
        if not residential_dict:
            residential_dict = kwargs.get('corporate_dict')
        if crm_lead_id:
            crm_lead_rec = request.env['crm.lead'].sudo().browse(int(crm_lead_id))
            for k, v in residential_dict.items():
                if k in ['passport_copy','company_licence','company_registration', 'articale_of_asociation','work_permit','salary_certificate','marriage_certificate'] and v:
                    type_rec = crm_lead_rec.lead_attachment_ids.filtered(lambda i: i.document_type==k)
                    type_rec.write({'contract_attachment': v, 'state': 're_review'})
        else:
            ...
            if kwargs.get('value_from') == 'Property page':
                final_attachment_lst = []
                for key, value in residential_dict.items():
                    if key in ['passport_copy','company_licence','company_registration', 'articale_of_asociation','work_permit','salary_certificate','marriage_certificate']:
                        final_attachment_lst.append((0, 0, {
                                'state': 'draft',
                ...
                inquiry = ' '
                property = request.env['account.asset'].sudo().browse(
                ...
                lead_id =  request.env['crm.lead'].sudo().create({
                    ...
                    'client_type': kwargs.get('rent_type'),
                    'lead_attachment_ids': final_attachment_lst,
                    'annual_company_income': residential_dict.get('annual_income'),
                    'civil_id': residential_dict.get('civil_id'),
                }).id
                return lead_id
